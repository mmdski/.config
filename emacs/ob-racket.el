...
(defcustom org-babel-command:racket "/opt/homebrew/bin/racket"
  ...
  (let* ((params (ob-racket--parse-params unparsed-params))
         ...
         (temporary-file-directory
          (file-name-as-directory
           (make-temp-file "ob-racket-" 'make-directory-only)))
         (main-filename
          (concat
           temporary-file-directory (make-temp-name "ob-racket") ".rkt")))
    ...
    (with-temp-buffer
      (prog2 (let ((default-directory temporary-file-directory))
  ...
  (let ((processed-params (org-babel-process-params params))
    ...
    (dolist (processed-param processed-params)
      (let ((key (car processed-param))
        ...
        (cond
         ((equal key :result-type)
          (setq result-type value))
         ((equal key :var)
          (push value vars))
         ((equal key :adjacent-file)
          (setq adjacent-files (split-string value "\s+")))
         ((equal key :require)
          (push value requires)))))
